...
class IDEHelperTest(unittest.TestCase):
    def testFindPYSMELLDICT(self):
        ...
        try:
            self.assertEqual(findPYSMELLDICT(os.path.join('a', 'random', 'path', 'andfile')), None, 'should not find PYSMELLTAGS')
            self.assertEqual(listdirs,
                [os.path.join('a', 'random', 'path'),
                 os.path.join('a', 'random'),
                 'a'], # two '' because of root again
                'did not list dirs correctly: %s' % listdirs)
            self.assertEqual(TRPArgs, [], 'did not read tags correctly')

            listdirs = []
            TRPArgs = []

            tags = findPYSMELLDICT(os.path.join('random', 'dirA', 'file'))
            self.assertEqual(tags, None, 'should not find pysmelltags')
            self.assertEqual(listdirs,
                [os.path.join('random', 'dirA'), 'random'],
                'did not list dirs correctly: %s' % listdirs)
            self.assertEqual(TRPArgs,
                [(os.path.join('random', 'dirA'), 'PYSMELLTAGS.django')],
    ...
    def testInferClassAbsolute(self):
        ...
        self.assertEqual(inferred, 'Nested.Package.Module.Class')
    ...
    def testInferClassYouDontKnowAbout(self):
        ...
        self.assertEqual(inferred, 'PackageB.NewModule.NewClass')
    ...
    def testInferClassParentsWithPointers(self):
        ...
        klass, parents = inferClass(os.path.join('TestData', 'PackageA', 'Module.py'),
            getSafeTree(source, 4), 4, NESTEDDICT)
        self.assertEqual(klass, 'PackageA.Module.Bother')
        self.assertEqual(parents, ['Nested.Package.Module.Class'])
...
class DetectOptionsTest(unittest.TestCase):
    ...
    def testInferShouldUpdatePYSMELLDICT(self):
        ...
        self.assertEqual(options, expected) #sanity
        klass = copiedDict['CLASSES']['PackageA.Module.FreshClass']
        self.assertEqual(klass['bases'], ['Nested.Package.Module.Class'])
        self.assertEqual(klass['properties'], ['something'])
        self.assertEqual(klass['methods'], [('sth', [], "")])
